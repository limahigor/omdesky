...
use omdesky_core::{KeyChord, KeyModifier, RemoteCommand, SessionRole, WindowSelector};
use std::sync::Arc;

pub const MOONLIGHT_WINDOW_CLASS: &str = "com.moonlight_stream.Moonlight";
const BINDS_GLOBAL: &str = "_G.omdesky_binds";
...
fn debug_notify(message: &str) -> String {
    format!(
        "hl.dispatch(hl.dsp.exec_cmd(\"omarchy-notification-send 'Omdesky debug' '{message}'\"))"
    )
}
...
omarchy-notification-send 'Omdesky debug' 'Could not send the remote shortcut'"
    );
    #[cfg(not(debug_assertions))]
...
{{ allow_input_capture = {capture}, description = \"Omdesky: {description}\" }}))",
...
mod tests {
    use super::*;
    use omdesky_application::ports::{ChildProcess, CommandOutput};
    ...
    fn test_controller_bind_dispatches_directly_without_http() {
        ...
        assert!(!spec.args[1].contains("omdesky command"));
        assert!(spec.args[1].contains("omarchy-notification-send 'Omdesky debug'"));
    ...
    fn test_remote_bind_forwards_to_controller_with_debug_only_notification() {
        ...
        assert!(!spec.args[1].contains("omdesky-keybind.log"));
        assert!(spec.args[1].contains("allow_input_capture = true"));
        assert!(!spec.args[1].contains("allow_input_capture = false"));
    }
    ...
    #[test]
    fn test_remove_clears_only_omdesky_bindings() {
        let spec = remove_session_keybinds_spec();

        assert!(spec.args[1].contains("omdesky_binds"));
