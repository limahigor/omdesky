...
impl Config {
    pub fn load() -> Result<Self, ConfigError> {
        ...
        if let Ok(port) = env::var("OMDESK_AGENT_PORT") {
            config.network.agent_port = port
                .parse()
                .map_err(|_| ConfigError::InvalidEnvironment("OMDESK_AGENT_PORT"))?;
        }
...
mod tests {
    ...
    fn test_save_to_persists_stream_settings() {
        let path = env::temp_dir().join(format!("omdesk-config-{}.toml", std::process::id()));
